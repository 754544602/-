var  sysCommon={
	...
	riskExcMark:"../../../../../resources/images/common/war-red.png",//风险感叹号图片
	riskWarImg:"../../../../../resources/images/common/risk-war.png",//风险未通过图片红灯
	riskViaImg:"../../../../../resources/images/common/risk-via.png",//风险通过图片绿灯
	...
	createFlowApproveMode:function(){
		var flowStr = '';
		    flowStr ='<div class="yt-first-title-model"><div class="yt-first-title">'
		            +'<label class="icon-flow-title"></label>'
		            +'<label class="yt-first-title-text">流程审批</label>'
		            +'</div>'
		            +'<div class="yt-first-title-line"></div></div>';//流程审批标题部分结束

		            //字段部分开始
		            flowStr +='<div class="ordinar-div operation">'
					...
					flowStr += '<div class="count-box" style="margin-left: 0px;padding-left: 0px;">'		
					        +'<div class="left">'
					        +'<span class="not-null opintion-msg-null" style="display: none;">*</span><span><span>操作意</span>见：</span>'
							+'</div>'	
							+'<div class="right">'	
							...
							+'</div></div></div></div>';
				    //给下拉列表做初始化设置
				    $(flowStr).find("select").niceSelect();
		    return flowStr;
	},
	/**
	 * 获取操作流程和审批人数据
	...
	getApproveFlowData:function(businessCode,processInstanceId,parameters){
		 ...
		 */
		 $.ajax({
				type: "get",
				url:$yt_option.websit_path+"json/flow-approve.json",
				async: false,
				data:{
	 ...
	 * 
	 */
	getLoginUserInfo:function(thisObj){
		//职务
		var  jobName = "";
		//电话
		var  telPhone = "";
		//判断职务和电话是否有值
		if($yt_common.user_info.jobName == undefined || $yt_common.user_info.jobName == "" || $yt_common.user_info.jobName ==null){
			jobName = '--';
		}else{
			jobName = $yt_common.user_info.jobName;
		}
		if($yt_common.user_info.userPhone == undefined || $yt_common.user_info.userPhone =="" || $yt_common.user_info.userPhone ==null){
			telPhone = "--";
		}else{
			telPhone = $yt_common.user_info.userPhone;
		}
		//判断是否传输区域对象
		var html = "";
		if(thisObj != undefined && thisObj !=null && thisObj !=""){
			//拼接用户信息代码
			  html='<div class="yt-form-users-info" style="padding-bottom: 10px;">'
	            +'<div class="yt-field-label">'
	            +'<label class="yt-read-text"><span style="letter-spacing:16px;">申请</span>人：</label>'
	            +'<span id="busiUsers">'+$yt_common.user_info.realName+'</span>'
	            +'</div>'
	            +'<div class="yt-field-label">'
	            +'<label class="yt-read-text"><span style="letter-spacing: 40px;">部</span>门：</label><span id="deptName">'+$yt_common.user_info.deptName+'</span>'
	            +'</div>'
	            +'<div class="yt-field-label">'
	            +'<label class="yt-read-text"><span style="letter-spacing: 40px;">职</span>务：</label><span id="jobName">'+jobName+'</span>'
	            +'</div>'
                +'<div class="yt-field-label">'
                +'<label class="yt-read-text"><span style="letter-spacing: 40px;">电</span>话：</label><span id="telPhone">'+telPhone+'</span>'
                +'</div></div>';
                $(thisObj).html(html);
		}else{
			$("#busiUsers").text($yt_common.user_info.realName);
    ...
    getAddressInfoList:function(){
    	...
    	$.ajax({
			type: "get",
			url:$yt_option.websit_path+'resources/js/system/expensesReim/module/reimApply/regionList.json',
			async: false,
			success: function(data) {
				dataList = data ;
			}
    ...
    getHotCityList:function(){
    	...
    	$.ajax({
			type: "get",
			url:$yt_option.websit_path+'resources/js/system/expensesReim/module/reimApply/hotCityList.json',
			async: false,
			success: function(data) {
				dataList = data ;
			}
        ...
        logQtipObj.qtip({  
            ...
            content: {  
                ...
                text: function(event,api) {  
					...
					$.ajax({
						type: "get",
						url: $yt_option.websit_path+"json/flowLog.json",
						async: false,
						data: {
							processInstanceId: processInstanceId
						},
	...
	getCommentByProcessInstanceId:function(processInstanceId){
		...
		$.ajax({
			type: "get",
			url: $yt_option.websit_path+"json/workFlow.json",
			async: false,
			data: {
				processInstanceId:processInstanceId
			},
	...
	processStatePop: function(processInstanceId) {
		//页面添加弹窗
		var iconUrl = $yt_option.websit_path+"resources/images/common/loop.png";
		var popHtml='<div class="yt-pop-model yt-edit-alert process-state-pop">'+
			'<div class="model-title">'+
			'<span class="yt-edit-alert-icon"><img src='+iconUrl+'></span>'+
			...
			'<button class="yt-assist-btn canel-btn" id="" style="margin-left: 10px;">关闭</button>'+
			'</div>'+
			'</div>'+
			'</div>';
		$("body").append(popHtml);
		...
		var imgUrl = $yt_option.websit_path+"resources/images/test/flow-state.png";
		...
		imgs.off().on("click", function() {
			...
			if($("#" + imgId).length > 0) {
				...
				closeElement.click(function() {
					$("#" + imgId).hide();
					//隐藏顶部隐藏蒙层
					$("#heard-nav-bak,#pop-modle-alert").hide();
					//调用隐藏全局蒙层的方法
				    $yt_baseElement.hideMongoliaLayer();
				});
 ...
 * 
-------html-----
<div class="yt-file-up"></div>

-------js-------
$(function() {
	$('.yt-file-up').fileUpLoad({
		titleName: '上传附件：',  //title名称
		fileUpId: '',  //文件input框的id  默认值为   ''
		fileBtnId: '', //请上传按钮id 默认值为  ''
		callback: function(fileData) {  //回调函数 返回文件数据
...
(function($) {
	var fileUpLoad = {
		...
		setHtml: function(obj, opt) {
			var inputId = opt.fileUpId || '';
			var buttonId = opt.fileBtnId || '';
			var html ='<table border="0" cellspacing="0" cellpadding="0" class="yt-file-table"><tbody><tr>' +
				'<td class="yt-file-title" style="width:'+(opt.titleWid || '100px')+'">' + (opt.titleName || '') + '</td>' +
				'<td><div class="file-up-div">' +
				'<div class="yt-input yt-file-hid-inpu"></div>' +
				'<div class="yt-file-main">' +
				'<input name="file" class="yt-cont-file" risk-code-val="'+opt.riskFontCode+'"  id="' + inputId + '" type="file">' +
				'<button class="yt-option-btn yt-common-btn file-upload-btn" id="' + buttonId + '" style="margin: 0;">请上传</button>';
				//判断是不是风险字段
				if(opt.riskFontCode !=undefined && opt.riskFontCode !="" && opt.riskFontCode !=null){
					//如果是的话拼接风险区域
					html+='<span class="risk-model">'
					     +'<input type="hidden" value="'+opt.riskFontCode+'" class="hid-risk-code"/>'
					     +'<img src="../../../../../riskLibrary/images/risk-def.png" class="risk-def-img"/>'
					     +'<img src="../../../../../riskLibrary/images/war-red.png" class="risk-img"/>'
					     +'</span>';
				}
				html+='</div>' +
				'</div>' +
				'<div class="file-box" id="attIdStr"></div>' +
				'</div>' +
				'</td>' +
				'</tr></tbody></table>';
			html = $(html);
		...
		setEvent: function(obj, opt) {
			var me = this;
			//附件上传
			me.fileUpEvent(obj, opt);
			//附件删除
			me.fileDelEvent(obj, opt);
		},
		fileUpEvent: function(obj, opt) {
			//上传附件
			obj.off().on('change', '.yt-cont-file', function() {
				var fileElementId = this.id;
				var ithisParent = obj.find('.file-box');
		...
		fileDelEvent: function(obj, opt) {
			//附件删除事件
			obj.find('.file-box').on('click', '.del-file', function() {
				var ithis = $(this);
	...
	$.fn.extend({
		fileUpLoad: function(opt) {
			...
			// opt.titleName title名称
			// opt.fileUpId 文件input框的id  默认值为 fileUp
			// opt.fileBtnId 上传按钮id 默认值为 fileBtn
			// opt.callback 回调函数 返回文件数据
			//整合参数配置
			var options = $.extend(defaults, opt);
			this.ytFileUpLoad = fileUpLoad;
			this.ytFileUpLoad.init($(this), options);
			return $(this);
		}
	});
})(jQuery);
