...
//主题对象
var themeObj = {
	theme_default: 'resources/css/common/yt-common.min.css', //默认
	theme_purple: "resources/theme/purple/yt-common.min.css", //紫色
	theme_red: "resources/theme/red/yt-common.min.css",
	lenghtG: 0
}
$(function() {
	...
	$(window).scroll(function(){
		...
		//获取菜单信息
		var menuEle = $(parentObj).find('.child-check .menu-element');
		var iJquery = window.parent.jQuery;  
		var $mydata = iJquery(menuEle);  
		var menuInfo = $mydata.data("menuData");
		var s = menuInfo.menuName;
		var i = s.lastIndexOf(')');
		var str = s.substr(1, i-1);
		if(i > 0) {
			if (themeObj.lenghtG == 1) {
				$left_menu.addBrowseRecord(str);
			}
		}
	});
});
...
var $yt_common = {
	...
	navFunEvent: function() {
		...
		//导航栏菜单鼠标悬浮事件
		$("#nav-fun-list li.them-li").mouseover(function() {
			$("#nav-fun-list li div").removeClass("nav-check nav-fun-check");
			$(this).find("div:not(.theme-panel)").addClass("nav-check nav-fun-check");
			$(this).find(".nav-fun-icon").attr("src", "./resources/images/common/cf2.png");
			//显示主题面板
			$(".theme-panel").show();
		}).mouseout(function() {
			$("#nav-fun-list li div").removeClass("nav-check nav-fun-check");
			$(this).find(".nav-fun-icon").attr("src", "./resources/images/common/cf1.png");
			//隐藏主题面板
			$(".theme-panel").hide();
		});
		/**
		 ...
		 * 
		 */
		$(".theme-panel .theme-div").off("click").on("click", function() {
			//调用冒泡方法
			$yt_baseElement.eventStopPageaction();
			//添加选中样式
			$(".theme-panel .theme-div").removeClass("theme-div-active");
			$(this).addClass("theme-div-active");
			//获取选中的主题code
			var themeCode = $(this).attr("theme-code");
			var themeUrl = "";
			switch(themeCode) {
				case "purple": //紫色
					themeUrl = themeObj.theme_purple;
					break;
				case "red": //红色
					themeUrl = themeObj.theme_red;
					break;
				default: //默认
					themeUrl = themeObj.theme_default;
			}
			//调用换肤方法
			$change_theme.changeThemes(themeUrl);
		});
		//鼠标移除事件
		$(".theme-panel").mouseleave(function() {
			//调用冒泡方法
			$yt_baseElement.eventStopPageaction();
			$(this).hide();
			//换主题标签样式切换
			$("#nav-fun-list li div").removeClass("nav-check nav-fun-check");
			//更换图标
			$(this).parent().find(".nav-fun-icon").attr("src", "./resources/images/common/cf1.png");
		});
	},
	/**
	 * 
	...
	getUserDataByDynamicKey: function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				//获取当前登录用户,并赋值
				$("#nav-fun-list .logo-user-name").text(data.data.userName);
				//用户头像
	...
	getUserInfo: function() {
		...
		$.ajax({
			type: "post",
			url: $yt_option.base_path + "index/getCurrentUserInfo",
			async: false,
			success: function(data) {
				$yt_common.user_info = data.data;
				//获取当前登录用户,并赋值
				$("#nav-fun-list .logo-user-name").text(data.data.realName);
				//用户头像
				/*var userPhoto = "./resources/images/common/user-photo-def.png";
				//判断是否有头像
				if(data.data.userHeadPictureToCol3!=undefined && data.data.userHeadPictureToCol3!=""){
...
var $left_menu = {
			...
			success: function(data) {
				...
				$.each(data.data.children, function(i, n) {
					...
					if(n.logoUrl == "" || n.logoUrl == undefined || n.logoUrl == null) {
						menuImgUrl = $yt_option.websit_path + "resources/images/common/menu-test.png";
					} else {
				...
				$(".menu-element").click(function() {
					...
					if(menuInfo.leaf == "true") {
						...
						/*if(menuInfo.menuUrl != "" && menuInfo.menuUrl !=" " && menuInfo.menuUrl !=null && menuInfo.menuUrl !=undefined){
							...
							$left_menu.switchTab(menuInfo.parentId,$websit_path+"resources/images/common/develop-page.png",menuInfo.pkId,menuInfo.openType,menuInfo.implateType,menuInfo.systemId);
						}*/
						var s = menuInfo.menuName;
						var i = s.lastIndexOf(')');
						var str = s.substr(1, i-1);
				...
				if(paramObj.pageUrl != "" && paramObj.pageUrl != null && paramObj.pageUrl != undefined) {
					...
					// $left_menu.switchTab(558,$websit_path+"view/system/expensesReim/module/busiTripApply/busiTripApply.html",511,1,2,55);
	...
	/**
	 * addBrowseRecord 浏览量明细增加
	 * @param {Object} str 菜单CODE
	 */
	addBrowseRecord: function(str) {
		$.ajax({
			type: "post",
			url: "widget/main/addBrowseRecord",
			async: false,
			data: {
				widgetCode: str
			},
	...
	switchTab: function(menuId, url, twoMenuId, openType, implateType, systemId) {
		...
		if(url == "" || $.trim(url).length <= 0 || url == undefined || url == null) {
			url = $websit_path + "resources/images/common/develop-page.png";
